...
namespace Yis 
{
	//  these unnamed namespaces are accessible within the file they're created in, 
	//  as if you had an implicit using-clause to them.
	namespace {
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			YS_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		if (!scene || !scene->HasMeshes())
			YS_CORE_ERROR("Failed to load mesh file: {0}", filename);

		aiMesh* mesh = scene->mMeshes[0];

		YS_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
		YS_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
		...
		{
			YS_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
