...
namespace Yis 
{
		...
		: m_Format(format), m_Width(width), m_Height(height)
	{
		...
		YS_RENDER_1(self, {
			...
			YS_CORE_INFO("Command Load 2D texture {0}", self->m_RendererID);
			});
	}
		...
		: m_FilePath(path)
	{
		...
		YS_CORE_INFO("Command Loading texture {0}, srgb={1}", path, srgb);
		...
		YS_RENDER_S1(srgb, {
			...
			if (srgb)
			{
				...
				YS_CORE_INFO("Creating srgb texture width {0} mips", levels);
			...
			else
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, YisToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, srgb ? GL_SRGB8 : YisToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData);
				glGenerateMipmap(GL_TEXTURE_2D);

				glBindTexture(GL_TEXTURE_2D, 0);
			}
			stbi_image_free(self->m_ImageData);
			YS_CORE_INFO("Command Loading texture {0}, srgb={1}", self->m_FilePath, srgb);
			});
	}
	...
	void OpenGLTexture2D::Bind(unsigned int slot) const
	{
		YS_RENDER_S1(slot, {
			...
			YS_CORE_INFO("Command Binding texture {0}", self->m_FilePath);
			});
	}
		...
		: m_FilePath(path)
	{
		...
		YS_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
		...
		YS_RENDER_S3(faces, faceWidth, faceHeight, {
			glGenTextures(1, &self->m_RendererID);
			...
			auto format = YisToOpenGLTextureFormat(self->m_Format);
			...
			stbi_image_free(self->m_ImageData);
			YS_CORE_INFO("Command Loading cube texture {0}", self->m_FilePath);
			});
	}
